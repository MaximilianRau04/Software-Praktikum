...
const fetchEventDetails = async () => {
  ...
  try {
    ...
    const organizerResponse = await fetch(`${config.apiBaseUrl}/api/events/${eventId}/organizer`);
    const organizerData = await organizerResponse.json();
    event.value.organizerId = organizerData.id;

    const exchangeDayResponse = await fetch(`${config.apiBaseUrl}/events/${eventId}/exchange-day`);
...
const fetchRegisteredUsers = async () => {
  const eventId = route.params.eventId;
  try {
    const response = await fetch(`${config.apiBaseUrl}/events/${eventId}/registeredUsers`);
...
watch(registeredUsers, (users) => {
  if (users.length > 0 && !users.some(user => user.id === event.value.organizerId)) {
    event.value.organizerId = users[0].id;
  }
});
</script>
