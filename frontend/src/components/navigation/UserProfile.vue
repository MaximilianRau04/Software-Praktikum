    ...
    <div class="profile-header">
      ...
      <div class="username-container">
        ...
        <div v-if="isEditing" class="edit-container">
          ...
          />
          <button @click="updateUsername" class="save-btn">
            Speichern
          </button>
        </div>
      </div>
    ...
    <div class="profile-section" v-if="registeredEvents.length">
      ...
      <ul class="scrollable-section">
        <li v-for="event in sortedEvents" :key="event.id" class="event-card">
          <h3>{{ event.name }}</h3>
          <p><strong>Datum:</strong> {{ formatDateTime(event.startTime) }} - {{ formatDateTime(event.endTime) }}</p>
    ...
    <div class="profile-section" v-if="forumPosts.length">
      ...
      <ul class="scrollable-section">
        <li v-for="post in sortedPosts" :key="post.id" class="forum-post">
          <p><strong>Thread:</strong> {{ threadTitles[post.threadId] || "Thread wird geladen..." }}</p>
...
export default {
  ...
  setup(props) {
    ...
    const sortedEvents = computed(() =>
      registeredEvents.value.sort((a, b) => new Date(b.startTime) - new Date(a.startTime))
    );
    ...
    const fetchUserData = async () => {
      try {
        const userResponse = await axios.get(`${apiUrl}users/search?username=${props.username}`);
    ...
    return { userData, registeredEvents, trainerProfile, forumPosts, threadTitles, formatDateTime, sortedEvents, sortedPosts, isEditing, newUsername, enableEdit, updateUsername };
  },
};
</script>
...
.event-card:hover, .forum-post:hover {
