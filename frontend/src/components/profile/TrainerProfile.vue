<template>
  <div>
    <button
      v-if="trainer.userId === getUserIdFromCookie()"
      class="manage-button"
    ...
    <div class="header-container">
      <div class="header-content">
        ...
        <TrainerHeader
          v-if="user && trainer"
          :user="user"
          :bio="trainer.bio || 'Keine Biografie verfügbar'"
        />
        <!-- Expertise Tags -->
    ...
    async fetchTrainerDetails() {
      try {
        const tagsResponse = await api.get(
          `/trainerProfiles/${this.trainer.id}/expertiseTags`,
        );
        ...
        const eventResponse = await api.get(
          `/users/${this.trainer.userId}/hostedEvents`,
        );
